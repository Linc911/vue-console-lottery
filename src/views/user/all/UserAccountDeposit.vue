  ...
  <section class="account-deposit">
    ...
    <div class="table-list">
      ...
      >
        ...
        <el-table-column prop="list" label="业务类型">
          <template slot-scope="scope">
            <template v-if="scope.row.list">
              <span v-for="(item, index) in scope.row.list" :key="index">{{item.payType === 1 ? '微信' : '其他' }} {{item.payNum | RMB}}</span>
            </template>
            <span v-else>本月无业务往来</span>
          </template>
        </el-table-column>
        <el-table-column prop="businessTime" label="业务时间" :min-width="140"></el-table-column>
      </el-table>
      <!-- 分页 -->
      <el-pagination
...
export default {
  ...
  methods: {
    ...
    fetchUserLogs (page) {
      this.$axios.get('/api-p/backend/statisticsUserOrder', {
